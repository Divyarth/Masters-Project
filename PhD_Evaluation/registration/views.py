...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        if studentProfessor == "student":
            ...
            studentName.objects.create(username=email, name=full_name)
        else:
            professorName.objects.create(username=email, name=full_name)
...
def editProfileStudent(request):
    if request.method == "POST":
        ...
        if studentProfile_Form.is_valid():
            ...
            studentName.objects.filter(username=sessionUserName).update(name=full_name)
